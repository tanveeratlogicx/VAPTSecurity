...
";
        ...
        // Add Config File
        $zip->addFromString( $folder . '/vapt-locked-config.php', $config_content );
        ...
        $exclude_list = [
            ...
            'ARCHITECTURE.md', 'CHANGELOG.md', 'DOCUMENTATION.md', 'FEATURES.md', 'VERSION_CONTROL.md', 'Project Layout.me', 'README.md', 'SUPERADMIN_GUIDE.md',
            'composer.json', 'composer.lock', 'package.json', 'package-lock.json', 'prompt.txt',
            'test-config.php', 'test-vapt-features.php',
            'tests', 'bin', 'node_modules',
        ...
        foreach ( $files as $name => $file ) {
            ...
            if ( $relative_path === 'vapt-locked-config.php' ) continue;
        ...
        if ( file_exists( $zip_file ) ) {
            ...
            if ( $count > 1 ) {
                 ...
                 if ( strlen( $last ) === 2 && in_array( $second_last, [ 'co', 'com', 'org', 'gov', 'net', 'edu', 'ac' ] ) ) {
    ...
    public function enforce_domain_lock( $is_activation = false ) {
        ...
        if ( ! isset( $vapt_locked_config_sig ) || ! hash_equals( $check_sig, $vapt_locked_config_sig ) ) {
            // Tampering detected!
            error_log( 'VAPT Security: Locked configuration file integrity check failed. File may have been tampered with.' );
            return;
        }
